                      ...
                      : PurchaseOrdersTable(
                          ...
                          onDelete: (order) async {
                            final confirmed = await showDeleteConfirmationDialog(
                              ...
                              message: context.translate(LangKeys.deletePurchaseOrderConfirmation),
                            );

                            if (confirmed != true || !context.mounted) return;

                            final success = await context.read<PurchaseOrdersCubit>().deletePurchaseOrder(order.id);

                            if (!context.mounted) return;

                            if (success) {
                              ShowToast.showToastSuccessTop(
                                message: context.translate(LangKeys.purchaseOrderDeletedSuccessfully),
                              );
                            }
                          },
                        ),
