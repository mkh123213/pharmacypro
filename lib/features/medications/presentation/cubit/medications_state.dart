...
class MedicationsState with _$MedicationsState {
  ...
  const factory MedicationsState.loaded({
    required List<MedicationModel> medications, @Default('') String searchQuery, @Default('all') String selectedCategory, @Default(false) bool isSubmitting,
  }) = MedicationsLoaded;
  const factory MedicationsState.failure({required String message}) = MedicationsFailure;
}
