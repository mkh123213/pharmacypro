...
class StaffCubit extends Cubit<StaffState> {
  ...
  Future<bool> createStaff(StaffModel staff, {required String password}) async {
    final current = state;
    ...
    try {
      final created = await _staffRepo.createStaff(staff, password: password);
  ...
  Future<bool> deleteStaff(String id) async {
    ...
    try {
      await _staffRepo.deleteStaff(id);

      _allStaff = oldStaff.where((item) => item.id != id).toList();
      ...
      emit(
        current.copyWith(
          staff: _filteredStaff,
          isSubmitting: false,
          errorMessage: 'could_not_delete_staff',
        ),
      );
  ...
  String _staffErrorMessage(Object error) {
    ...
    if (text.contains('staff_password_too_short')) {
      return 'staff_password_too_short';
    }

    if (text.contains('staff_auth_email_already_in_use')) {
      return 'staff_auth_email_already_in_use';
    }

    if (text.contains('staff_auth_account_creation_failed')) {
      return 'staff_auth_account_creation_failed';
    }
