                    ...
                    : InventoryTable(
                        ...
                        onDelete: (item) async {
                          final confirmed = await showDeleteConfirmationDialog(
                            context: context,
                            title: context.translate(LangKeys.deleteInventoryItem),
                            message: context.translate(LangKeys.deleteInventoryItemConfirmation),
                          );

                          if (confirmed != true || !context.mounted) return;

                          final success = await context.read<InventoryCubit>().deleteInventoryItem(item.id);

                          if (!context.mounted) return;

                          if (success) {
                            ShowToast.showToastSuccessTop(
                              message: context.translate(LangKeys.inventoryItemDeletedSuccessfully),
                            );
                          }
                        },
                      ),
