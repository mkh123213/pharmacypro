                            ...
                            onDelete: state.isSubmitting
                                ...
                                : () async {
                                    final confirmed = await showDeleteConfirmationDialog(
                                      context: context,
                                      title: context.translate(LangKeys.deleteCustomerOrder),
                                      message: context.translate(LangKeys.deleteCustomerOrderConfirmation),
                                    );

                                    if (confirmed != true || !context.mounted) return;

                                    final success = await context.read<CustomerOrdersCubit>().deleteCustomerOrder(order.id);

                                    if (!context.mounted) return;

                                    if (success) {
                                      ShowToast.showToastSuccessTop(
                                        message: context.translate(LangKeys.customerOrderDeletedSuccessfully),
                                      );
                                    }
                                  },
                          );
