...
class CustomerOrdersCubit extends Cubit<CustomerOrdersState> {
  ...
  Future<bool> deleteCustomerOrder(String id) async {
    final current = state;

    if (current is! CustomerOrdersLoaded || current.isSubmitting) return false;
    ...
    try {
      await _customerOrdersRepo.deleteCustomerOrder(id);

      _allOrders = oldOrders.where((item) => item.id != id).toList();
      ...
      emit(
        current.copyWith(
          orders: _filteredOrders,
          isSubmitting: false,
          errorMessage: 'could_not_delete_customer_order',
        ),
      );
