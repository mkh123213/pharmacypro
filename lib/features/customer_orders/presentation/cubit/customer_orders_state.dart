...
class CustomerOrdersState with _$CustomerOrdersState {
  ...
  const factory CustomerOrdersState.loaded({
    required List<CustomerOrderModel> orders, required List<MedicationModel> medications, required List<BranchModel> branches, @Default('') String searchQuery, @Default('all') String selectedStatus, @Default(false) bool isSubmitting,
  }) = CustomerOrdersLoaded;
  const factory CustomerOrdersState.failure({required String message}) = CustomerOrdersFailure;
}
